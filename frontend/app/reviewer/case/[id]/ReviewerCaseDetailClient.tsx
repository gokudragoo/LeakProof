...
export default function ReviewerCaseDetailClient({ caseId }: { caseId: number }) {
  ...
  const handleDecryptReporterSeverity = async () => {
    setError('');
    if (!encryptedSeverity) {
      setError('Encrypted severity is not available yet.');
      return;
    }
    if (!cofheReady) {
    ...
    try {
      setStatusLine('Decrypting reporter severity with your permit...');
      setReporterSeverity(await decryptHandle(encryptedSeverity));
      setStatusLine('');
    } catch (decryptError) {
      setStatusLine('');
      setError(decryptError instanceof Error ? decryptError.message : 'Unable to decrypt reporter severity.');
    }
  };

  const handleDecryptVoteSummary = async () => {
    setError('');
    if (!encryptedSummaryHandles) {
      setError('Encrypted vote summary is not available yet.');
      return;
    }
    if (!cofheReady) {
      setError('The confidential client is still connecting.');
    ...
    try {
      setStatusLine('Decrypting confidential vote summary...');
      ...
      setStatusLine('');
      setError(decryptError instanceof Error ? decryptError.message : 'Unable to decrypt vote summary.');
    }
  };
        ...
        {!isConnected ? (
          ...
          {(statusLine || error) && (
            <div className={`mb-6 p-4 rounded-lg border flex items-center gap-3 ${
              ...
              {!error ? <div className="w-5 h-5 rounded-full border-2 border-blue-500 border-t-transparent animate-spin" /> : null}
              {error || statusLine}
            </div>
          )}
            ...
            <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {reporterSeverity === null ? (
                        <button
                          type="button"
                          onClick={handleDecryptReporterSeverity}
                          disabled={!cofheReady}
                          className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-sky-300 hover:bg-white/10 disabled:opacity-50"
                        >
                          Decrypt
                        </button>
                      ) : null}
              ...
              <div className="glass-card p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                  <h3 className="text-xl font-semibold">Current Vote Tally</h3>
                  <button
                    type="button"
                    onClick={handleDecryptVoteSummary}
                    disabled={!cofheReady || !encryptedSummaryHandles}
                    className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-sky-300 hover:bg-white/10 disabled:opacity-50"
                  >
                    Decrypt Summary
                  </button>
                </div>
