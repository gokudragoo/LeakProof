...
export default function ReviewerCaseDetail({ params }: { params: { id: string } }) {
  ...
  const handleSubmitVote = async (e: React.FormEvent) => {
    ...
    const encryptedScore = encryptField(voteData.severityScore.toString(), key);
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 relative z-10">
        {submitted || isSuccess ? (
          <div className="text-center py-20 slide-up">
            ...
            <p className="text-gray-400 mb-8">Your confidential vote has been recorded on-chain.</p>
            <Link
              href="/reviewer/dashboard"
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-400 hover:to-pink-400 text-white font-semibold transition-all hover-lift"
            >
              Back to Dashboard
            </Link>
          </div>
              ...
              <form onSubmit={handleSubmitVote} className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {[
                      ...
                      <button
                        key={rec.id}
                        type="button"
                        onClick={() => setVoteData({ ...voteData, recommendation: rec.id })}
                        className={`py-4 px-6 rounded-xl font-semibold transition-all duration-300 hover-lift flex flex-col items-center gap-2 ${
                          voteData.recommendation === rec.id
                            ? rec.color === 'emerald' ? 'bg-gradient-to-br from-emerald-500 to-teal-500 text-white scale-105' :
                              rec.color === 'red' ? 'bg-gradient-to-br from-red-500 to-rose-500 text-white scale-105' :
                              'bg-gradient-to-br from-amber-500 to-orange-500 text-black scale-105'
                            : 'bg-dark-800 border border-gray-700 hover:border-purple-500'
                        }`}
                      >
                ...
                <div className="glass rounded-2xl p-6 slide-up animate-delay-200">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-4 rounded-xl bg-dark-800 border border-gray-700 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition-all duration-300 min-h-[150px] resize-none"
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="w-full py-4 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-400 hover:to-pink-400 disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed text-white font-semibold transition-all duration-300 hover-lift flex items-center justify-center gap-2 button-glow"
                >
                  {isPending ? (
                    <>
